...
must use five newly spawned reviewer subagents, and the loop continues until
all reviewers in the newest pass report no actionable findings.

## Workflow

1. Finish the local change and run relevant verification first.
2. Spawn exactly five new subagents for review. Never reuse an existing or
...
8. Repeat until all five reviewers in the newest pass report no actionable
   findings.
9. In the final handoff, state the verification command and that the final fresh
   five-reviewer subagent pass had no actionable findings.

## Reviewer Prompt Template

Use a prompt like this for each reviewer in a pass:
...
- Fix every actionable finding from every reviewer before final handoff,
...
- After any local fix, do not ask the same reviewers to re-check. Spawn five
  new subagents for the next pass.

## Stop Condition

Stop only when all five reviewers in the newest fresh subagent pass report no
actionable findings.
